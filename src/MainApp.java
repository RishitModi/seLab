...
public class MainApp {
    public static void main(String[] args) {
        ...
        while (running) {
            ...
            if ("1".equals(choice)) {
                                ...
                                WhiteBoxTestRunner runner = new WhiteBoxTestRunner();
                                runner.runAll();
    ...
    private static void printMenu() {
        System.out.println("--------------------------------------------------");
                System.out.println("1. Register Complaint (Manual Testing)");
                System.out.println("2. Run Black Box Tests");
                System.out.println("3. Run White Box Tests");
                System.out.println("4. Exit");
        System.out.print("Select an option: ");
    }

        private static void runManualMode(Scanner scanner) {
                System.out.print("Enable simulation mode? (y/n): ");
                boolean simulationMode = readYesNo(scanner, "", false);

                boolean systemAvailable = true;
                boolean agentAvailable = true;
                if (simulationMode) {
                        boolean simulateSystemDown = readYesNo(scanner, "Simulate system down? (y/n): ", false);
                        boolean simulateNoAgent = readYesNo(scanner, "Simulate no agent available? (y/n): ", false);
                        systemAvailable = !simulateSystemDown;
                        agentAvailable = !simulateNoAgent;
                }
                ...
                ComplaintController controller = new ComplaintController(
                                new ComplaintService(systemAvailable, agentAvailable)
                );

        ComplaintResult result = controller.registerComplaintResult(customerId, details);
        System.out.println("--------------------------------------------------");
        if (result.isSuccess()) {
            Complaint complaint = result.getComplaint();
                        String assignedTo = "Agent";
            ...
            System.out.println("Ticket ID: " + complaint.getId());
                        System.out.println("SLA: Assigned");
                        System.out.println("Assigned To: " + assignedTo);
                        System.out.println("Status: " + status);
        } else {
                        System.out.println("Error: " + formatError(result));
        }
        System.out.println("--------------------------------------------------");
    }

        private static boolean readYesNo(Scanner scanner, String prompt, boolean defaultValue) {
                while (true) {
        ...
        private static String formatError(ComplaintResult result) {
                if (result.getStatus() == ComplaintResult.Status.INVALID_CUSTOMER) {
                        return "Invalid customer";
                }
                if (result.getStatus() == ComplaintResult.Status.INVALID_COMPLAINT) {
                        return "Invalid complaint";
                }
                if (result.getStatus() == ComplaintResult.Status.SYSTEM_UNAVAILABLE) {
    ...
    private static class TestRunner {
        ...
        private boolean runTestCase(TestCase testCase) {
            ...
            for (String expected : testCase.expectedLines) {
                System.out.println("  " + expected);
            }

            TestOutcome outcome = execute(testCase);
            printStepByStepComparison(testCase.expectedLines, outcome.actualLines);
        ...
        private boolean matchesExpected(String[] expected, String[] actual) {
            ...
            return true;
                ...
                private void printStepByStepComparison(String[] expected, String[] actual) {
                        System.out.println("Step-by-step comparison:");
                        int index = 0;
                        for (int i = 0; i < expected.length; i++) {
                                String expectedLine = expected[i];
                                int matchIndex = -1;
                                for (int j = index; j < actual.length; j++) {
                                        if (actual[j].contains(expectedLine)) {
                                                matchIndex = j;
                                                index = j + 1;
                                                break;
                                        }
                                ...
                                if (matchIndex >= 0) {
                                        System.out.println("  Step " + (i + 1) + " OK");
                                        System.out.println("    Expected: " + expectedLine);
                                        System.out.println("    Actual: " + actual[matchIndex]);
                                } else {
                                        System.out.println("  Step " + (i + 1) + " FAIL");
                                        System.out.println("    Expected: " + expectedLine);
                        ...
                        if (actual.length == 0) {
                                System.out.println("  Actual output lines: (none)");
                                return;
                        }

                        System.out.println("  Actual output lines (all):");
